            ...
            <div class="list_module">
              <div class="table_body">
                  <el-table border  :data="tableData"  size='small' style="width: 100%" :empty-text="$t('noData')">
                    ...
                    <el-table-column prop="userName" :label="$t('artificial.ApprovalPerson')" align="center" min-width="205px"> </el-table-column><!-- 审批人员 -->
                    <el-table-column prop="productName" :label="$t('artificial.productName')" align="center" min-width="145px"> </el-table-column><!-- 产品名称 -->
                    <el-table-column prop="acceptDate" :label="$t('artificial.TaskGenerationDate')" align="center" min-width="180px"> <!-- 任务生成日期 -->
                    </el-table-column>
                    <el-table-column prop="acceptTime" :label="$t('artificial.TaskGenerationTime')" align="center" min-width="180px"> <!-- 任务生成时间 -->
                    </el-table-column>
                    <el-table-column :label="$t('artificial.operation')" width="200px" align="center" v-if="funList.indexOf('调件') > -1 " min-width="140px"><!-- 操作 -->
            ...
            <el-dialog :title="title" :visible.sync="dialogFormVisible"  :before-close="cancel" :close-on-click-modal="false">
              <el-form :label-position="$store.state.lang == 'en' ? 'top' : 'right'" :model="addForm" :rules="$rules.loanArtificialSent" ref="addForm" id="addForm" size="small"  label-width="150px">
                    ...
                    <el-form-item :label="$t('artificial.fpspyh')" prop="loanArtSentUser" ><!-- 分配审批用户 -->
                      <el-select v-model="addForm.loanArtSentUser" filterable>
                        <el-option 
...
export default {
  ...
  methods:{
    ...
    initLoanArtSent () {
      this.$post(this.$approve + '/manualApproval/manualApprovalList',this.obj).then(res =>{
        if(res.code == '000000'){
    ...
    thingThat(row){
      ...
      var obj={
      	...
      	"productNum":this.row.productNum,
      	"limit":this.row.amount,
        "legalPersonNum":this.obj.legalPersonNum,
        "taskUserId":row.userId
        
      }
      this.$post(this.$approve + '/manualApproval/userlList',obj).then(res =>{
          if(res.code == '000000'){
    ...
    submitAdd(formName){
      this.$refs[formName].validate((valid) => {
       ...
       var FunMsgData={
          ...
          FunMsgData.userId=this.addForm.loanArtSentUser;
          this.$post(this.$approve + '/manualApproval/adjustingParts',FunMsgData).then(res=>{
            if(res.code=="000000"){
