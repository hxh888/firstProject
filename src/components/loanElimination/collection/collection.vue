  ...
  <div class="content_box" id="contract_content">
    ...
    <div class="list_module">
	    	<el-tabs v-model="state"> 
                <el-tab-pane :label="$t('collection.Collection_List_Query')" name="0"><!-- 催收列表查询 -->
                    <template>
                      <el-table border
                        ...
                        <el-table-column prop="loanNo" :label="$t('collection.Debit_Number')" align="center" min-width="115px"> </el-table-column><!-- 借据编号 -->
                        <el-table-column prop="productName" :label="$t('collection.Product_Name')" align="center" min-width="115px"></el-table-column><!-- 产品名称 -->
                        <!-- <el-table-column prop="iousBalance" :label="$t('collection.Debit_balance')" align="center" min-width="115px"> </el-table-column>借据余额 -->
                        <el-table-column prop="phone" :label="$t('collection.Contact_number')" align="center" min-width="130px"> </el-table-column><!-- 联系电话 -->
                        <el-table-column prop="collectType" :label="$t('collection.Suggested_collection_method')" align="center" min-width="200px"><!-- 建议催收方式 -->
                          <template slot-scope="scope">
                            {{scope.row.collectType | level(collectTypeList)}}
                          </template>
                        </el-table-column>
                        <el-table-column :label="$t('Operation')" align="center" v-if="funList.indexOf('处理') > -1" min-width="110px"><!-- 操作 -->
                          <template slot-scope="scope">
                            <el-button type="primary" size="small" @click="editItem(scope.row)">{{$t('collection.Handle')}}</el-button><!-- 处理 -->
                          </template>
                        </el-table-column>
    ...
    <el-dialog :title="addForm.flag ? $t('See') : $t('collection.Registration_of_Collection_Processing')" :visible.sync="addUserDialog" :before-close="cancels"  :close-on-click-modal="false" width="800px">  
        ...
        <el-row class="edit_module_search" v-if="!addForm.flag">
          ...
          <el-col :span="12">
                <el-form-item :label="$t('collection.Collection_method')" :prop="!(cnhkrq && cnhkrq > now) ? 'collectType' : ''">   <!-- 催收方式 -->
                    <el-select v-model="addForm.collectType" style="width: 100%" :disabled="cnhkrq && cnhkrq > now" placeholder="">
          ...
          <el-col :span="12">
                <el-form-item :label="$t('collection.Date_of_implementation')"   :prop="!(cnhkrq && cnhkrq > now) ? 'executeDate' : ''">   <!-- 执行日期 -->
                  <el-date-picker v-model="addForm.executeDate" type="date" style="width:100%" value-format="yyyy-MM-dd" ref="executeDate" :disabled="cnhkrq && cnhkrq > now"></el-date-picker>
                </el-form-item>
          </el-col>		
        
          <el-col :span="12">
                <el-form-item :label="$t('collection.Collection_conclusion')" :prop="!(cnhkrq && cnhkrq > now) ? 'collectConclusion' : ''">   <!-- 催收结论 -->
                    <el-select v-model="addForm.collectConclusion" style="width: 100%;" @change="collectConclusionChange" :disabled="cnhkrq && cnhkrq > now" placeholder="">
          ...
          <el-col :span="12" v-if="addForm.collectConclusion == '0'">
                <el-form-item :label="$t('collection.Date_of_Commitment_to_Payment')" :prop="!(cnhkrq && cnhkrq > now) ? 'promiseRepayDate' : ''">   <!-- 承诺还款日期 -->
                  <el-date-picker v-model="addForm.promiseRepayDate" type="date" style="width:100%" value-format="yyyy-MM-dd" :picker-options="pickerOptions" @focus="cnhkrqChange" :disabled="cnhkrq && cnhkrq > now"></el-date-picker>
                </el-form-item>
          </el-col>		

          <el-col :span="12" v-if="addForm.collectConclusion == '0'">  
                <el-form-item :label="$t('collection.Amount_of_promised_repayment')" :prop="!(cnhkrq && cnhkrq > now) ? 'promiseRepayAmount' : ''"> <!-- 承诺还款金额 -->
                    <el-input v-model="addForm.promiseRepayAmount" :disabled="cnhkrq && cnhkrq > now"></el-input>
                </el-form-item>
          </el-col>	

          <el-col :span="24">
                <el-form-item :label="$t('collection.Explanation_of_collection_conclusion')" prop="collectConclusionRemark">   <!-- 催收结论说明 -->
                  <el-input type="textarea" v-model="addForm.collectConclusionRemark" maxlength="100" show-word-limit :disabled="cnhkrq && cnhkrq > now"></el-input>
                </el-form-item>
          </el-col>	
        </el-row> 
        <el-row>
        ...
        <el-row>
          ...
          <div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="sure('addForm')" v-if="!addForm.flag" :disabled="loading || (cnhkrq && cnhkrq > now)">{{$t('Save')}}</el-button><!-- 保存 -->
          </div>
      </el-dialog>
    <!--新增弹框页面结束-->
...
export default {
    ...
    return {
      ...
      collectConclusionList: [],
	    rules:{
          ...
          // collectConclusionRemark: [
          //   { required: true, message: '催收结论说明不能为空', trigger: 'change' },
          // ],
          promiseRepayAmount:[
  ...
  methods:{
    // 请先选择执行日期
    cnhkrqChange () {
      if (!this.addForm.executeDate) {
        this.$alert(i18n.t('collection.Please_select_the_execution_date'), i18n.t('Prompt'), {
          dangerouslyUseHTMLString: true
        });
        this.$refs.executeDate.focus();
      }
    },
    // 承诺还款日期大于执行日期
    disabledDate (time) {
        return time.getTime() < new Date(this.addForm.executeDate).getTime() - 60*60*24*1000
    },
    ...
    editItem (row) {
      ...
      this.addForm.collectConclusionRemark = ''
      ...
      setTimeout(function() {
        self.$post(self.$afterloan + '/collect/findDetail',{taskNo: row.taskNo, legalPersonNum: self.$store.state.loginData.userLogin.legalPersonNum, collectPerson: self.$store.state.loginData.userLogin.loginId}).then(res =>{
          if(res.returnCode == '000000'){
            ...
            self.userForm.address = res.returnData.address
            self.userForm.collectHisRows = res.returnData.collectHisRows
            self.cnhkrq = res.returnData.newPromiseRepayDate ? new Date(res.returnData.newPromiseRepayDate).getTime() : '0'
          } else {
            var str = `
              <p>${ i18n.t('Error_code')}: <span style='color:#5bc0de'>${res.returnCode}</span></p>
              <p >${ i18n.t('reason')}:<span style='color:#5bc0de;'>${res.returnMsg}</span></p>
