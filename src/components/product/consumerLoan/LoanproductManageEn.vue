				...
				<el-row>
					...
					<el-form-item>
						<el-button type="warning" icon="el-icon-search" @click="initLoanProductList()" >查询</el-button>  <!--查询按钮-->
			  			<el-button type="primary" icon="el-icon-plus"  @click="productAdd()">新增</el-button> <!--新增按钮-->
					</el-form-item>
					</el-col>
				</el-row>  
			</el-form>
...
export default {
  ...
  methods:{
	...
	//新增跳转
	productAdd() {
		this.$router.push({path:'/MainIndex/credit/productManageAdd?id=2'})
	},
	//查看
	seeItem(row){
		...
		this.$post(this.$api,productforViewData).then(res=>{
			if(res.returnMsg=="OK"){
				...
				this.$router.push({path:'/MainIndex/credit/productManageView?id=2'})
				this.$store.commit('saveProductManageView',res); //保存登录信息  
			}else{
				this.$message({
					message:res.returnMsg,
					type: 'warning' 
